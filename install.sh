...
install_system_deps() {
    ...
    case "$PLATFORM" in
        ...
        macos)
            ...
            brew install python@3.11 cmake qt@6
            ;;
    esac
...
setup_ahk() {
    ...
    if [[ -n "$WIN_USER" ]]; then
        ...
        cp "$SCRIPT_DIR/scripts/launch.ahk" "$AHK_DEST" 2>/dev/null || true
        
        if [[ -f "$AHK_DEST" ]]; then
            log_success "AutoHotkey script installed: $AHK_DEST"
            log_info "To enable: Install AutoHotkey v2 and run the script"
        fi
    else
...
print_summary() {
    ...
    echo ""
    if $IS_WSL2; then
        echo -e "  ${YELLOW}Windows hotkey:${NC}"
        echo -e "    Install AutoHotkey v2 and run the script in Documents/AutoHotkey/"
        echo -e "    Then press Ctrl+Shift+A from anywhere to launch"
        echo ""
    fi
    echo -e "  ${YELLOW}Next steps:${NC}"
    echo -e "    1. Download a GGUF model to ~/models/"
    echo -e "    2. Set the model path in $CONFIG_DIR/config.json"
